...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WCXCOINMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("WCXCOINMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("WCXCOINMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running WCXCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
